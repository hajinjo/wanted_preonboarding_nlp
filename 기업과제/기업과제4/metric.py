...
def final_rouge_score(df):
    # 요약 데이터 불러오기 
    df

    # rouge score 불러오는 함수 
    def get_rouge(predict, refer):

        rouge = RougeScorer()
        score= rouge.compute_rouge(predict, refer)
        score = score.split(",")
        ...
        return round(rouge,3)

    # score return
    def metric():
        title = df.iloc[:,0].tolist()
        label = df.iloc[:,1].tolist()
        score = [get_rouge(title[i], label[i]) for i in range(len(title))]
        rouge = [score[i] for i in range(len(score))]

        return rouge
 

    df["rouge"] = metric()
    df_score = df.iloc[:,1:3]
    df_score.to_csv("./score.csv", header=['SUMMARY', 'SCORE'], index=False)
    print(df_score)

    metric_mean = df["rouge"].sum()/len(df)
    print("ROUGE Score 평균 :", metric_mean)
    return df
